...
if ((INSTALL_BACKEND)); then
    ...
    grep -q 'Gain(Format.FLOAT, 10 \*\* (-6 / 20))' "$SOURCE_DIR/backend/csdr/chain/dablin.py" || die "DAB float-domain headroom is missing"
...
PY
    ...
    grep -q 'Gain(Format.FLOAT, 10 \*\* (-6 / 20))' "$DAB_CHAIN_TARGET" || die "installed DAB float-domain headroom is missing"
