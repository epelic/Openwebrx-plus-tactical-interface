    ...
    [true,false].forEach(function(isAnalog,index){
      ...
      classified.forEach(function(entry){
        ...
        b.addEventListener('click',function(){
          function activate(value){var nativeButton=entry.index===0&&nativeModeButton(host,value);if(nativeButton)nativeButton.click();else if(String(select.value)!==String(value))triggerSelect(select,value);setTimeout(syncModeButtons,25)}
          activate(opt.value);
        });
        keys.appendChild(b);
      });
