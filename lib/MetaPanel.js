...
function WfmMetaPanel(el) {
    ...
    this.timeout = false;
    var me = this;
    this.fmLevelHandler = function(event) {
        var detail = event.detail || {};
        function meter(value) {
            value = Math.max(value || 0, .001);
            return Math.max(0, Math.min(100, (60 + 20 * Math.log10(value)) * 100 / 60));
        }
        var $el = $(me.el);
        $el.find('.rds-vu-left').css('width', meter(detail.left || detail.level) + '%');
        $el.find('.rds-vu-right').css('width', meter(detail.right || detail.level) + '%');
        $el.find('.rds-stereo-state').text(detail.stereo ? 'STEREO' : 'MONO')
            .toggleClass('active', !!detail.stereo);
        $el.find('.rds-mpx-difference').css('width', meter(detail.difference) + '%');
    };
    window.addEventListener('fm-audio-level', this.fmLevelHandler);
    this.fmReceptionTimer = setInterval(function() { me.updateReceptionInfo(); }, 750);
    this.clear();
}

WfmMetaPanel.prototype = new MetaPanel();

function toggleFmStereo(button) {
    var mono = button.getAttribute('data-mono') !== '1';
    button.setAttribute('data-mono', mono? '1' : '0');
    button.textContent = mono? 'MONO' : 'STEREO';
    button.classList.toggle('active', mono);
    if (typeof audioEngine !== 'undefined' && audioEngine && audioEngine.setFmMono) audioEngine.setFmMono(mono);
}

function setFmStereoWidth(input) {
    var value = Math.max(0, Math.min(100, parseInt(input.value, 10) || 0));
    $(input).siblings('.fm-width-value').text(value + '%');
    if (typeof audioEngine !== 'undefined' && audioEngine && audioEngine.setFmStereoWidth) audioEngine.setFmStereoWidth(value / 100);
}

function setFmBandwidth(input) {
    var khz = Math.max(50, Math.min(190, parseInt(input.value, 10) || 150));
    $(input).siblings('.fm-bandwidth-value').text(khz + ' kHz');
    if (typeof UI === 'undefined' || !UI.getDemodulator) return;
    var demodulator = UI.getDemodulator();
    if (!demodulator || demodulator.get_modulation() !== 'wfm' || !demodulator.setBandpass) return;
    var half = khz * 500;
    demodulator.setBandpass({low_cut: -half, high_cut: half});
    if (UI.saveBandpass) UI.saveBandpass('wfm', -half, half);
}

WfmMetaPanel.prototype.updateReceptionInfo = function() {
    var snr = '--';
    if (window.spectrum && spectrum.data && spectrum.data.length > 40) {
        var values = spectrum.data.filter(function(v) { return Number.isFinite(v); });
        var n = values.length;
        if (n > 40) {
            var median = function(a) { var b = a.slice().sort(function(x, y) { return x - y; }); return b[Math.floor(b.length / 2)]; };
            var signal = median(values.slice(Math.floor(n * .42), Math.ceil(n * .58)));
            var noise = median(values.slice(0, Math.floor(n * .12)).concat(values.slice(Math.ceil(n * .88))));
            if (Number.isFinite(signal) && Number.isFinite(noise)) snr = Math.max(0, signal - noise).toFixed(1) + ' dB';
        }
    }
    $(this.el).find('.rds-snr').text('S/N: ' + (snr === '--' ? snr : '≈ ' + snr));
    if (typeof UI !== 'undefined' && UI.getDemodulator) {
        var demodulator = UI.getDemodulator();
        if (demodulator && demodulator.get_modulation() === 'wfm' && Number.isFinite(demodulator.low_cut) && Number.isFinite(demodulator.high_cut)) {
            var khz = Math.round((demodulator.high_cut - demodulator.low_cut) / 1000);
            $(this.el).find('.fm-bandwidth input').val(khz);
            $(this.el).find('.fm-bandwidth-value').text(khz + ' kHz');
        }
    }
};
...
WfmMetaPanel.prototype.update = function(data) {
    if (!this.isSupported(data)) return;
    if (typeof data.deviation === 'number') {
        $(this.el).find('.rds-deviation').text('DEV: ' + data.deviation.toFixed(1) + ' kHz');
    }
    if (Array.isArray(data.mpxSpectrum)) {
        this.drawMpxSpectrum(data.mpxSpectrum);
    }
    if (Object.keys(data).every(function(key) {
        return key === 'mode' || key === 'deviation' || key === 'mpxSpectrum';
    })) {
        return;
    }
    this.rdsMpxSeenUntil = Date.now() + 10000;
    var me = this;
...
WfmMetaPanel.prototype.drawMpxSpectrum = function(spectrum) {
    var canvas = $(this.el).find('.rds-mpx-spectrum')[0];
    if (!canvas || !spectrum.length) return;
    if (!this.mpxPeakSpectrum || this.mpxPeakSpectrum.length !== spectrum.length) {
        this.mpxPeakSpectrum = spectrum.slice();
    } else {
        for (var peakIndex = 0; peakIndex < spectrum.length; peakIndex++) {
            this.mpxPeakSpectrum[peakIndex] = Math.max(
                spectrum[peakIndex], this.mpxPeakSpectrum[peakIndex] - 0.8
            );
        }
    }
    spectrum = this.mpxPeakSpectrum;
    var pixelRatio = window.devicePixelRatio || 1;
    var displayWidth = Math.max(1, Math.round(canvas.clientWidth));
    var displayHeight = Math.max(1, Math.round(canvas.clientHeight));
    ...
    context.strokeStyle = '#183b22';
    context.lineWidth = 1;
    [19, 38, 57, 76, 92].forEach(function(frequency) {
        var x = Math.round(frequency / 92 * (width - 1)) + 0.5;
        context.beginPath();
        context.moveTo(x, 0);
        context.lineTo(x, height);
        context.stroke();
    });
    context.strokeStyle = '#72ff8b';
    context.lineWidth = 1.5;
    context.beginPath();
    spectrum.forEach(function(value, index) {
        var x = index / (spectrum.length - 1) * (width - 1);
        var y = (height - 1) * (1 - Math.max(0, Math.min(100, value)) / 100);
    ...
    if (enabled) {
        $(this.el).removeClass('disabled').html(
            '<div class="rds-container">' +
                ...
                '</div>' +
                '<div class="rds-stereo-state">MONO</div>' +
                '<div class="rds-mpx-readout"><span class="rds-snr">S/N: --</span><span class="rds-deviation">DEV: -- kHz</span></div>' +
                '<div class="rds-mpx-spectrum-wrap"><span>MPX PEAK HOLD · 0–92 kHz</span><canvas class="rds-mpx-spectrum"></canvas>' +
                    '<div class="rds-mpx-scale"><span style="left:0">0</span><span style="left:20.65%">19</span><span style="left:61.96%">57 RDS</span><span style="right:0">92 kHz</span></div>' +
                '</div>' +
                '<div class="rds-vu"><span>L</span><i><b class="rds-vu-left"></b></i></div>' +
                '<div class="rds-vu"><span>R</span><i><b class="rds-vu-right"></b></i></div>' +
                '<div class="fm-width"><span>STEREO WIDTH</span><input type="range" min="0" max="100" value="65" oninput="setFmStereoWidth(this)"><b class="fm-width-value">65%</b></div>' +
                '<div class="fm-width fm-bandwidth"><span>RF BANDWIDTH</span><input type="range" min="50" max="190" step="5" value="150" oninput="setFmBandwidth(this)"><b class="fm-bandwidth-value">150 kHz</b></div>' +
                '<button type="button" class="dab-stereo-toggle fm-stereo-toggle" data-mono="0" onclick="toggleFmStereo(this)">STEREO</button>' +
...
WfmMetaPanel.prototype.clear = function() {
    ...
    $(this.el).find('.rds-vu-left, .rds-vu-right, .rds-mpx-difference').css('width', 0);
    $(this.el).find('.rds-stereo-state').text('MONO').removeClass('active');
    $(this.el).find('.rds-snr').text('S/N: --');
    $(this.el).find('.rds-deviation').text('DEV: -- kHz');
    var canvas = $(this.el).find('.rds-mpx-spectrum')[0];
    if (canvas) canvas.getContext('2d').clearRect(0, 0, canvas.width, canvas.height);
    this.mpxPeakSpectrum = null;
};
...
function toggleDabStereo(button) {
    var mono = button.getAttribute('data-mono') !== '1';
    button.setAttribute('data-mono', mono? '1' : '0');
    button.textContent = mono? 'MONO' : 'STEREO';
    button.classList.toggle('active', mono);
    if (typeof audioEngine !== 'undefined' && audioEngine && audioEngine.setDabMono) audioEngine.setDabMono(mono);
}
...
function DabMetaPanel(el) {
    ...
    this.$select.on("change", function() {
        ...
        me.updateDabStationInfo();
    });
    var $container = $(
        '<div class="dab-container"><div class="dab-info-column">' +
            ...
            '<div class="dab-programme-slot"></div>' +
            '<div class="dab-technical-grid">' +
                '<span>Service ID</span><b data-dab-field="service_id">--</b>' +
                '<span>Subchannel</span><b data-dab-field="subchannel">--</b>' +
                '<span>Codec</span><b data-dab-field="codec">--</b>' +
                '<span>Audio</span><b data-dab-field="audio">--</b>' +
                '<span>Bitrate</span><b data-dab-field="bitrate">--</b>' +
                '<span>Protection</span><b data-dab-field="protection">--</b>' +
                '<span>Capacity</span><b data-dab-field="capacity">--</b>' +
                '<span>Component</span><b data-dab-field="component">--</b>' +
            '</div></div>' +
            '<div class="dab-logo-column dab-station-column"><div class="dab-logo-title">DAB STATION</div>' +
                '<div class="dab-station-name">SELECT A STATION</div>' +
                '<div class="dab-radiotext"><span>NOW PLAYING</span><b>WAITING FOR DLS…</b></div>' +
                '<div class="dab-logo-reception"><div class="dab-logo-block">DAB BLOCK: --</div>' +
                '<div class="dab-logo-signal">S/N: -- · RF: -- dB</div></div>' +
                '<div class="dab-vu"><div class="dab-vu-row"><span>L</span><i><b class="dab-vu-left"></b></i></div>' +
                '<div class="dab-vu-row"><span>R</span><i><b class="dab-vu-right"></b></i></div></div>' +
                '<button type="button" class="dab-stereo-toggle" data-mono="0" onclick="toggleDabStereo(this)">STEREO</button>' +
            '</div></div>'
    );
    $container.find('.dab-programme-slot').append(this.$select);
    $(this.el).append($container);
    this.clear();
    this.programmeTimeout = false;
    this.logoInfoTimer = setInterval(function() { me.updateLogoReceptionInfo(); }, 750);
    this.dabLevelHandler = function(event) {
        var d = event.detail || {};
        var level = function(value) { return Math.max(0, Math.min(100, (60 + 20 * Math.log10(Math.max(value || 0, .001))) * 100 / 60)); };
        $(me.el).find('.dab-vu-left').css('width', level(d.left) + '%');
        $(me.el).find('.dab-vu-right').css('width', level(d.right) + '%');
    };
    window.addEventListener('dab-audio-level', this.dabLevelHandler);
};
...
DabMetaPanel.prototype.updateLogoReceptionInfo = function() {
    var profile = ($('#openwebrx-sdr-profiles-listbox option:selected').text() || '').trim();
    var block = /\bDAB\s*([0-9]{1,2}[A-D])\b/i.exec(profile);
    var snr = '--';
    if (window.spectrum && spectrum.data && spectrum.data.length > 40) {
        var values = spectrum.data.filter(function(v) { return Number.isFinite(v); });
        var n = values.length;
        if (n > 40) {
            var median = function(a) { var b = a.slice().sort(function(x, y) { return x - y; }); return b[Math.floor(b.length / 2)]; };
            var signal = median(values.slice(Math.floor(n * .18), Math.ceil(n * .82)));
            var noise = median(values.slice(0, Math.floor(n * .1)).concat(values.slice(Math.ceil(n * .9))));
            if (Number.isFinite(signal) && Number.isFinite(noise)) snr = Math.max(0, signal - noise).toFixed(1) + ' dB';
        }
    }
    var rf = ($('#openwebrx-smeter-db').text() || '').trim().replace(/\s*dB\s*$/i, '');
    $(this.el).find('.dab-logo-block').text('DAB BLOCK: ' + (block? block[1].toUpperCase() : '--'));
    $(this.el).find('.dab-logo-signal').text('S/N: ' + (snr === '--'? snr : '≈ ' + snr) + ' · RF: ' + (rf || '--') + ' dB');
};

DabMetaPanel.prototype.updateDabStationInfo = function() {
    var name = (this.$select.find('option:selected').text() || '').trim();
    $(this.el).find('.dab-station-name').text(!name || name === 'Loading...'? 'SELECT A STATION' : name);
};
...
DabMetaPanel.prototype.update = function(data) {
    ...
    if ('ensemble_id' in data) {
        var ensembleId = data.ensemble_id;
        var formattedEnsembleId = typeof ensembleId === 'number'
            ? '0x' + ensembleId.toString(16).toUpperCase()
            : String(ensembleId).replace(/^0x/i, '0x');
        $(this.el).find('.dab-ensemble-id').text(formattedEnsembleId);
    }
    ...
    if ('dab_details' in data) {
        var d = data.dab_details || {};
        var values = {
            service_id: d.service_id,
            subchannel: d.subchannel,
            codec: d.codec || d.service_type,
            audio: [d.sample_rate? (d.sample_rate / 1000) + ' kHz' : '', d.audio_mode || (d.channels? d.channels + ' ch' : '')].filter(Boolean).join(' · '),
            bitrate: (d.audio_bitrate || d.subchannel_bitrate)? (d.audio_bitrate || d.subchannel_bitrate) + ' kbit/s' : '',
            protection: d.protection,
            capacity: d.cu_size? d.cu_size + ' CU @ ' + d.cu_start : '',
            component: d.component
        };
        Object.keys(values).forEach(function(key) {
            $(this.el).find('[data-dab-field="' + key + '"]').text(values[key] || '--');
        }, this);
        $(this.el).find('.dab-radiotext b').text(d.radiotext || 'WAITING FOR DLS…');
        this.updateLogoReceptionInfo();
    }
...
DabMetaPanel.prototype.clear = function() {
    ...
    $(this.el).find('[data-dab-field]').text('--');
    $(this.el).find('.dab-station-name').text('SELECT A STATION');
    $(this.el).find('.dab-radiotext b').text('WAITING FOR DLS…');
    $(this.el).find('.dab-vu-left,.dab-vu-right').css('width', 0);
    this.updateLogoReceptionInfo();
};
...
function DrmMetaPanel(el) {
    ...
    $(this.el).append($container);
    var me = this;
    $(this.el).on('click keydown', '.drm-program[data-audio-service]', function(event) {
        if (event.type === 'keydown' && event.key !== 'Enter' && event.key !== ' ') return;
        event.preventDefault();
        var id = parseInt($(this).attr('data-audio-service'), 10);
        if (isNaN(id)) return;
        me.service_id = id;
        UI.getDemodulator().setAudioServiceId(id);
        $(me.el).find('.drm-program').removeClass('selected');
...
DrmMetaPanel.prototype.update = function(data) {
    ...
    window.dispatchEvent(new CustomEvent('drm-audio-lock', {
        detail: { locked: data.status.msc > 0 }
    }));
...
//    }
    ...
    if (data.service_list) {
        ...
        for (var j = 0 ; j < data.service_list.length ; j++) {
            ...
            programs +=
                '<div class="drm-program' + (entry.is_audio && j == this.service_id? ' selected' : '') + '"' +
                    (entry.is_audio? ' data-audio-service="' + j + '" role="button" tabindex="0" title="Select this DRM audio service"' : '') + '>' +
