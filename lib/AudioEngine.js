...
function AudioEngine(maxBufferLength, audioReporter) {
    ...
    this.dabHeadroom = Math.pow(10, -9 / 20);
}
...
AudioEngine.prototype.pushHdAudio = function(data) {
    ...
    var stereo = modulation === 'dab' || modulation === 'wfm' || modulation === 'cquam';
...
StereoResampler.prototype.process = function(data) {
    ...
    var outputFrames = Math.max(1, Math.floor(frames * ratio));
    var output = new Float32Array(outputFrames * 2);
    for (var j = 0; j < outputFrames; j++) {
        var position = j / ratio;
        var a = Math.min(frames - 1, Math.floor(position));
        var b = Math.min(frames - 1, a + 1);
        var mix = position - a;
        output[j * 2] = ((data[a * 2] * (1 - mix) + data[b * 2] * mix) + .5) / 32768;
        output[j * 2 + 1] = ((data[a * 2 + 1] * (1 - mix) + data[b * 2 + 1] * mix) + .5) / 32768;
    }
    return output;
};
